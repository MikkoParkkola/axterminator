...
use crate::accessibility::{
    attributes, get_children, get_string_attribute_value, AXUIElementRef,
};
...
// ---------------------------------------------------------------------------
// Extraction helpers — each ≤ 25 lines
// ---------------------------------------------------------------------------

fn extract_app_context(app_ref: AXUIElementRef, children: &[AXUIElementRef]) -> AppContext {
    AppContext {
...
// ---------------------------------------------------------------------------
// Fine-grained attribute readers
// ---------------------------------------------------------------------------

fn find_focused_window(app_ref: AXUIElementRef) -> Option<String> {
...
fn first_window_ref<'a>(
    app_ref: AXUIElementRef,
    children: &'a [AXUIElementRef],
        ...
        if role == "AXTabGroup" {
            if let Ok(tab_children) = get_children(c) {
                return tab_children.iter().find_map(|&tc| {
                    let selected = crate::accessibility::get_bool_attribute_value(
                        tc,
                        "AXSelected",
                    )?;
                    if selected {
...
fn find_active_document(children: &[AXUIElementRef]) -> Option<String> {
    // Try windows first, then document attribute on the app element itself.
    children.iter().find_map(|&c| {
...
fn find_selected_text(win: AXUIElementRef) -> Option<String> {
    // Walk children looking for a focused text area / text field with selected text.
        ...
        if get_string_attribute_value(el, attributes::AX_ROLE).as_deref() == Some("AXList") {
            if let Ok(rows) = get_children(el) {
                for (i, &row) in rows.iter().enumerate() {
                    let selected = crate::accessibility::get_bool_attribute_value(row, "AXSelected")
                        .unwrap_or(false);
                    if selected {
                        return Some(i);
                    }
...
fn collect_selected_items_in(elements: &[AXUIElementRef]) -> Vec<String> {
    ...
    for &el in elements {
        if crate::accessibility::get_bool_attribute_value(el, "AXSelected").unwrap_or(false) {
...
fn build_breadcrumb(children: &[AXUIElementRef]) -> Vec<String> {
    // Collect window → toolbar → scroll area hierarchy titles as breadcrumbs.
...
fn find_role_selection_in(elements: &[AXUIElementRef], target_role: &str) -> Option<String> {
    for &el in elements {
        if get_string_attribute_value(el, attributes::AX_ROLE).as_deref() == Some(target_role) {
            if let Ok(rows) = get_children(el) {
                for &row in &rows {
                    if crate::accessibility::get_bool_attribute_value(row, "AXSelected")
                        .unwrap_or(false)
                    {
    ...
    for &el in elements {
        ...
        if matches!(
                ...
                .or_else(|| get_string_attribute_value(el, attributes::AX_TITLE))
            {
                ...
                buf.push_str(&truncate(val, remaining));
            }
        }
        if let Ok(kids) = get_children(el) {
