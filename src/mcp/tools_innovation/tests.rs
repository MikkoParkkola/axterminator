...
// GIVEN: action=write but no text field, in normal mode
// WHEN: dispatching
let result =
        super::handle_ax_clipboard_with_mode(&json!({"action": "write"}), SecurityMode::Normal);
...
let result = super::handle_ax_clipboard_with_mode(
...
#[test]
fn ax_session_info_security_mode_matches_explicit_mode() {
// GIVEN: sandboxed mode set by the caller
let registry = Arc::new(AppRegistry::default());
// WHEN: calling the handler
let result =
        super::handle_ax_session_info_with_mode(&json!({}), &registry, SecurityMode::Sandboxed);
...
#[test]
fn ax_session_info_tool_count_matches_visible_tools_for_mode() {
// GIVEN: a fresh registry
let registry = Arc::new(AppRegistry::default());
// WHEN: calling the handler in safe mode
let result = super::handle_ax_session_info_with_mode(&json!({}), &registry, SecurityMode::Safe);
let v: serde_json::Value = serde_json::from_str(&result.content[0].text).unwrap();
// THEN: tool_count matches the visible tool set for that mode
assert_eq!(
        v["tool_count"].as_u64().unwrap() as usize,
        crate::mcp::tools::tools_for_mode(SecurityMode::Safe).len()
);
}
