...
pub(crate) fn call_system_tool(name: &str, args: &Value, _mode: &str) -> ToolCallResult {
    match name {
        "ax_fs_read" => {
            ...
            match std::fs::read_to_string(&path) {
                Ok(content) => {
                    let len = content.len();
                    ToolCallResult::ok(
                        json!({ "path": path, "size_bytes": len, "content": content }).to_string(),
                    )
                }
        ...
        "ax_fs_write" => {
            ...
            match result {
                Ok(n) => {
                    ToolCallResult::ok(json!({ "path": path, "bytes_written": n }).to_string())
                }
        ...
        "ax_fs_list" => {
            ...
            ToolCallResult::ok(
                json!({ "path": path, "entries": entries, "count": count }).to_string(),
            )
        }
        "ax_process_list" => {
            let filter = args
