...
mod tests {
    ...
    fn innovation_tools_registers_fifteen_tools() {
        // GIVEN: Wave 2 + ax_record + ax_analyze + ax_run_script
        //        + ax_clipboard + ax_session_info + ax_undo + ax_visual_diff + ax_a11y_audit
        //        (workflow tools extracted to tools_workflow module)
        // WHEN: requesting descriptors
        let tools = super::innovation_tools();
        // THEN: exactly twelve tools registered
        assert_eq!(
            tools.len(),
            12,
            "expected 12 innovation tools, got {}",
            tools.len()
        );
    }
    ...
    fn innovation_tool_names_match_expected_set() {
        ...
        // Note: ax_workflow_* moved to crate::mcp::tools_workflow
        for expected in &[
            "ax_query",
            "ax_app_profile",
            "ax_test_run",
